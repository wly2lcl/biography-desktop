...
> 最后更新: 2026-07-17（macOS 未签名 Release 修复完成，待远端重跑）
...
| **验证结果** | 已修复 Release run `29564331322` 暴露的空 identity 问题；发布元数据、构建策略、YAML、316 项前端测试及生产构建均通过 |
| **发布状态** | 工作流修复已完成但尚未远端重跑；上一轮 Windows/Linux 成功，macOS Intel/Apple Silicon 失败 |
...
| 6 | 测试与打包 | 90% | 🔨 前端测试 316/316 / Rust 默认及实验各 61 项 / Linux 打包 ✅ / Windows CI/CD ✅ / macOS 修复完成待远端重跑 |
...
- 2026-07-17 的手动 Release 证明通用构建步骤向未签名 macOS runner 注入了空 `APPLE_SIGNING_IDENTITY`，导致 Tauri 调用 `codesign --sign ""`；现已拆分签名与未签名步骤，静态门禁与本地构建通过，仍需远端 macOS runner 复验。
...
| 任务总数 | 309 个（含 Phase 11 产品化、十一轮提交前复审及 macOS Release 修复任务） |
| 已完成任务 | 286 个（92.6%） |
