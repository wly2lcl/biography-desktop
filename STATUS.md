...
> 最后更新: 2026-07-20（v0.1.1 本地发布准备完成，待提交、推送与远端构建）
...
| **当前阶段** | 🔨 Phase 11 公开稳定产品化实施中 |
| **验证结果** | v0.1.1 版本一致性、发布元数据、构建策略、316 项前端测试、75.5% 行覆盖率、稳定/实验构建及默认/`local-model` Rust 门禁通过 |
| **发布状态** | v0.1.1 本地准备完成；提交并推送后，手动 Release 将从清单自动创建标签，正式发布仍需外部证书与公证验收 |
...
| 6 | 测试与打包 | 90% | 🔨 前端测试 316/316 / Rust 默认及实验各 61 项 / 四目标构建 ✅ / macOS 签名与公证待凭据及远端复验 |
...
| 11 | 公开稳定产品化 | 80% | 🔨 本地产品化实施完成，原生 E2E 与签名发布待外部验收 |

> Phase 9 表示功能代码已实现，不代表达到正式发布安全标准。本地模型在 Phase 10 正式构建中默认关闭。

### Phase 11 当前结论

- 项目版本已统一升级到 0.1.1；手动 Release 已取消版本输入，改为从提交中的清单自动生成 `v0.1.1`，避免再次出现双版本来源。
- 本地产品化已完成真实重试、启动降级恢复、Rust LLM transport、密钥边界、稳定/实验隔离、上下文预算、静态示例与诊断导出。
- 第十轮已修复 Web 请求边界的精确作用域密钥读取和诊断包原始错误栈泄露；其门禁结果不再作为当前工作区可提交结论。
- 第十一轮已修复 SSE 跨 chunk CRLF 边界和临时内存模式的数据管理语义，并把 `local-model` feature 纳入 CI 实际编译测试。
- 2026-07-17 的第二次手动 Release 已证明空 `APPLE_SIGNING_IDENTITY` 构建失败得到修复，Windows、Linux、macOS Intel/Apple Silicon 四目标均构建成功。
- 同次 Release 因仓库没有任何 Actions 签名 Secrets，两个 macOS 目标都跳过证书导入并生成 ad-hoc/未公证应用；下载后被 Gatekeeper 显示为“已损坏”，不能视为可公开安装的 macOS 产物。
- Release 已按平台独立判定 Apple 与 Windows 凭据：任一平台凭据完整即可生成该平台签名产物；全局 `signed` 仅控制正式稳定版，仍要求用户请求稳定发布且两组凭据均完整。
- 本地已通过发布元数据组合测试、构建策略检查、YAML 解析、316 项前端测试、75.5% 行覆盖率及稳定/实验生产构建；GitHub 远端签名与公证仍需配置 Apple Secrets 后复验。
- 仍未闭环真实原生桌面 E2E、Windows/macOS 签名与公证、干净系统安装/升级/卸载及回滚验证。
- Windows/macOS 签名、公证与干净系统安装/升级/卸载属于外部人工门禁，当前不得标记为稳定发布完成。
- 产品默认定位为隐私优先 BYOK；不建设账号、云同步、计费、业务后端或默认遥测。
...
- [x] 第七轮复审：禁止 Rust 请求重定向，修复跨 chunk UTF-8 与响应头前取消。
- [x] 第七轮复审：API Key 按 provider/custom Base URL 隔离，实验 custom 改走 Rust transport。
- [x] 第七轮复审：补齐设置回滚和传记实际生成元数据。
- [x] 第七轮复审：301 项前端、两套 54 项 Rust、覆盖率与稳定/实验构建门禁通过。
- [x] 第八轮复审：传记生成元数据在 SQLite、导入导出和 v2/v3 备份恢复中完整往返。
- [x] 第八轮复审：修复 LLM 预取消、Retry-After、密钥作用域、传记预算、世界复制和 IPv6 回环问题。
- [x] 第八轮复审：306 项前端、两套 60 项 Rust、覆盖率与稳定/实验构建门禁通过，全工作区复查未发现新的可操作问题。
- [x] 第九轮复审：隔离设置页未保存密钥草稿，防止 provider/Base URL 切换后误发。
- [x] 第九轮复审：稳定前端产物彻底排除本地模型 IPC 与事件实现，并修复产物检查漏检。
- [x] 第九轮复审：系统模式失败后保留真实重试所需参数。
- [x] 第九轮复审：完成当轮门禁；后续第十轮复审发现 Web 作用域密钥请求边界和诊断隐私仍需修复。
- [x] 第十轮复审：Web 空草稿只加载当前 provider/Base URL 的精确作用域密钥，显式草稿优先。
- [x] 第十轮复审：可分享诊断包排除原始错误栈并脱敏 Promise rejection reason。
- [x] 第十轮复审：315 项前端测试、覆盖率、稳定/实验构建、两套 Rust 门禁及全工作区复查通过。
- [x] 第十一轮收口：修复 Rust SSE 跨 chunk CRLF 事件边界。
- [x] 第十一轮收口：临时内存模式禁用误导性的持久化数据操作和磁盘数据库信息。
- [x] 第十一轮收口：CI 实际编译、检查并测试 `local-model` feature，同时构建实验前端。
- [x] 第十一轮收口：完整门禁和全工作区复审通过，未发现新的 P0/P1 阻塞项，确认可以提交。
...
- 当前已重新执行 292 项 TypeScript 测试并全部通过；全局行覆盖率 71.16%，稳定/实验前端构建与组合配置检查通过。48 项 Rust 独立测试、Rust fmt 与默认/`local-model` feature Clippy `-D warnings` 均已重新通过。
...
| 定义的 Tauri Commands | 稳定版 32 个；实验版额外注册 10 个本地模型命令 |
| 世界观文件数 | 2 个（已迁移 wuxia_jianghu.md） |
| 系统方案文件数 | 3 个（已迁移） |
| Prompt 模板数 | 6 个（已迁移 + 文件系统覆盖支持） |
| 任务总数 | 311 个（含 Phase 11 产品化、十一轮提交前复审及 Release 平台签名修复任务） |
| 已完成任务 | 286 个（92.6%） |
...
| 测试文件数 | 30 个（316 个测试用例，全部通过 ✅） |
| Rust 单元测试 | 默认及 `local-model` 各 61 个独立测试（lib/bin 目标均通过 ✅） |
