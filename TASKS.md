...
| T322 | Windows 打包（.exe/.msi） | P0 | ⏸️ | 1h | - | 需 Windows 环境 |
| T323 | macOS 打包（.app/.dmg） | P0 | ⏸️ | 1h | - | 需 macOS 环境 + 开发者签名 |
| T324 | Linux 打包（.deb/.AppImage） | P0 | ✅ | 1h | - | ✅ .deb + .rpm 打包成功 |
| T325 | 打包产物验证 | P0 | ✅ | 2h | - | ✅ .deb 6.1MB 验证通过（含二进制/图标/desktop 文件） |
...
| Phase 6: 测试与打包 | 2 天（TS 测试 ✅ 81 tests / Linux 打包 ✅ / Windows⏸️ / macOS⏸️） |
| P0 阻塞问题修复 | ~1 天 ✅ |
| P1 建议改进 | ~2 天（8/10 完成） |
| P2 长期优化 | ~2.5 天（7/8 完成） |
| **实际完成** | **约 15-16 天** |
| **剩余工作量** | **约 0.5 天（仅 Windows/macOS 打包验证 + 7 项集成测试）** |
...
│
Phase 6 (测试与打包) ~70%
├── T300-T312 测试 ← Phase 2-5（TS 测试 ✅ 81 tests / 集成测试 🔲 需 Tauri+LLM / Rust 测试代码已写好 🔲 需 cargo）
│   └── T320-T325 打包（图标 ✅ / Linux .deb+.rpm ✅ / Windows ⏸️ / macOS ⏸️ / 产物验证 ✅）
```

> **注**: 剩余 7 项待完成任务全部需要特定运行环境（Tauri 运行时 + 真实 LLM API + Windows/macOS 构建），非代码开发工作。代码层面 + Linux 打包开发已全部完成。
