...
> **隐私提示**：云端模式会把生成所需的文本上下文发送给所选模型提供商。Web 模式仅用于开发调试，API Key 会落入浏览器存储，不作为正式交付方式。
>
...
| **测试** | Vitest（227 个单元与本地 mock 集成测试）+ Rust（45 项独立测试） |
...
- ✅ Linux 打包完成（.deb + .rpm）
- ✅ CI/CD 工作流配置完成
- ✅ 当前依赖与 Rust 工具链下已验证 227 项前端测试、45 项 Rust 独立测试及稳定/实验本地组合构建；全局行覆盖率 66.18%
