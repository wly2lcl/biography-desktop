...
const BIOGRAPHY_PROMPT_BASE = `你是一位传奇传记作家。根据以下信息，为这位人物撰写一篇完整的传记。
...
const BIOGRAPHY_PROMPT_INCOMPLETE = `\n\n【特别注意】这段旅程尚未完结，主角的故事在途中暂停。
...
class PromptManager {
  ...
  biographyPrompt(isComplete: boolean = true): string {
    return isComplete
      ? BIOGRAPHY_PROMPT_BASE
      : BIOGRAPHY_PROMPT_BASE + BIOGRAPHY_PROMPT_INCOMPLETE;
  }
